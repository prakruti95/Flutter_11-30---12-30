...
class MyDbHelper
{
  ...
  Future<int>insertdata2(Map<String, dynamic> row)async
  {
    Database? db = await instance.database;
  ...
  {
    Database db = await instance.database;
    return await db.query(table1);//select * from category
  }

  Future<List<Map<String,dynamic>>>querycotact()async
  {
      Database db = await instance.database;
  ...
  Future<int> deletedata(int id) async
  {
    ...
    //table1,where: '$columnId1 = ?', whereArgs: [id]
  }
}